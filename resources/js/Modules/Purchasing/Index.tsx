        ...
        <AppLayout>
            ...
            <div className="space-y-4">
                <div className="flex items-center justify-between">
                    <h1 className="text-xl font-bold text-white">أوامر الشراء</h1>
                    <Link href="/purchase-orders/create" className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm transition-colors">
                ...
                <div className="flex gap-2">
                    {['', 'draft', 'confirmed', 'received', 'cancelled'].map(s => (
                        <button key={s} onClick={() => router.get('/purchase-orders', s ? { status: s } : {})}
                        ...
                        <tbody className="divide-y divide-slate-800">
                            {orders.data.map(o => (
                                <tr key={o.id} className="hover:bg-slate-800/50">
                                    ...
                                    <td className="px-4 py-3">
                                        <Link href={`/purchase-orders/${o.id}`} className="text-slate-400 hover:text-blue-400"><Eye size={15} /></Link>
                                    </td>
                                </tr>
                            ))}
                            {orders.data.length === 0 && (
                                <tr><td colSpan={6} className="px-4 py-8 text-center text-slate-500">لا توجد أوامر شراء</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
