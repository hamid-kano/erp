        ...
        <AppLayout>
            ...
            <div className="space-y-4">
                <div className="flex items-center justify-between">
                    <h1 className="text-xl font-bold text-white">فواتير الشراء</h1>
                ...
                <div className="flex gap-3">
                    <select value={filters.status ?? ''} onChange={e => router.get('/purchase-invoices', { ...filters, status: e.target.value })}
                    ...
                    <select value={filters.supplier_id ?? ''} onChange={e => router.get('/purchase-invoices', { ...filters, supplier_id: e.target.value })}
                        className="bg-slate-900 border border-slate-700 text-white rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-blue-500">
                        ...
                        <tbody className="divide-y divide-slate-800">
                            {invoices.data.map(inv => (
                                <tr key={inv.id} className="hover:bg-slate-800/50">
                                    ...
                                    <td className="px-4 py-3 text-white">{inv.supplier?.name}</td>
                                    ...
                                    <td className="px-4 py-3">
                                        <Link href={`/purchase-invoices/${inv.id}`} className="text-slate-400 hover:text-blue-400">
                                            <Eye size={15} />
                                        </Link>
                                    </td>
                                </tr>
